class Admin::NewsController < Admin::BaseController
  ...
  def create
    ...
    respond_to do |format|
      if @new.save
        flash[:notice] = I18n.t('new.create.success').capitalize
        format.html { redirect_to(admin_news_path(@new)) }
        format.xml  { render :xml => @new, :status => :created, :location => @new }
      else
        flash[:error] = I18n.t('new.create.failed').capitalize
  ...
  def update
    ...
    respond_to do |format|
      if @new.update_attributes(params[:new])
        flash[:notice] = I18n.t('new.update.success').capitalize
        format.html { redirect_to(admin_news_path(@new)) }
        format.xml  { head :ok }
      else
        flash[:error] = I18n.t('new.update.failed').capitalize
  ...
  def destroy
    @new = New.find(params[:id])
    if @new && @new.destroy
      flash[:notice] = I18n.t('new.destroy.success').capitalize
    else
      flash[:error] = I18n.t('new.destroy.failed').capitalize
    end
